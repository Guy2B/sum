...
(() => {
  ...
  function initMail(ctx) {
    ...
    function renderAccounts() {
      ...
      const hasRealGoogleAccount = accounts.some(
        (account) => account.provider === 'gmail' && account.demo === false
      );
      document.getElementById('mail-mode-label').textContent =
        API || hasRealGoogleAccount ? ctx.t('mail.secureMode') : ctx.t('mail.demoMode');
    }
    ...
    document.getElementById('mail-sync').addEventListener('click', async () => {
      ...
      const gmailAccount = (ctx.getState().mailAccounts || [])
        .find((account) => account.provider === 'gmail');

      if (window.SigmaGoogle?.configured?.() && gmailAccount) {
        try {
          const messages = await window.SigmaGoogle.importGmail();
          ctx.updateState((state) => {
            const currentAccount = (state.mailAccounts || [])
              .find((account) => account.provider === 'gmail');

            state.mailAccounts = (state.mailAccounts || [])
              .filter((account) => account.provider !== 'gmail');

            state.mailAccounts.push({
              id: 'google-gmail',
              provider: 'gmail',
              email: currentAccount?.email || gmailAccount.email || '',
              label: 'Gmail',
              demo: false,
              status: 'connected',
